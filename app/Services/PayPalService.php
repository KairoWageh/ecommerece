...
class PayPalService
{
	...
	{
	    if (config('paypal.paypal_client_id') == '' || config('paypal.paypal_secret_id') == '') {
	    ...
	    $this->payPal = new ApiContext(
	        new OAuthTokenCredential(
	            ...
	            config('paypal.paypal_secret_id')
	        )
	    );
	...
	{
		//shipping amount if there is charge for shipping, now 0
		...
		foreach ($order->items as $item)
		{
		    ...
		    $orderItems[$item->id]->setName($item->product->title)
		        ->setCurrency(config('paypal.currency_code'))
		...
		$redirectUrls->setReturnUrl(route('cart.payment.complete'))
